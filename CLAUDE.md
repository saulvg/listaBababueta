...
3. Modelo de datos (resumen)
   ...
   lists y products llevan además position (entero): el orden que dejan los padres arrastrando las tarjetas en el panel. NO es un número de fila — se permiten huecos y negativos, porque una lista nueva entra en (mínimo − 1) para quedarse arriba y un regalo nuevo en (máximo + 1) para quedarse al final, y así el alta es una escritura y no N. Todas las consultas ordenan por [position asc, createdAt] con la fecha de desempate, que es el orden que tenían antes.
   lists lleva también hidden (booleano): la lista deja de verse en /listas Y su enlace directo pasa a responder "esa lista no existe", aunque quien llame tenga la clave. Ver sección 5 para el porqué de que sea invisible y no "no listada".
...
5. Decisiones de arquitectura (el porqué, no solo el qué)
   ...
   El paso a COMPRADO solo ocurre por markProductAsPurchased (lib/products/purchase.ts). El PATCH de productos de los padres NO puede tocar status, purchased_by ni purchased_at — el esquema de Zod ni siquiera acepta esos campos —, porque un PATCH genérico que escribiera el estado sería una puerta trasera a la garantía anterior. Los padres pueden marcar y desmarcar desde el panel, pero por un endpoint propio (POST y DELETE de /api/admin/products/[id]/purchase) que llama a las mismas funciones de lib/products/purchase.ts: toda escritura de status pasa por ese módulo, sea de la familia o de los padres. Desmarcar usa la misma actualización condicional (solo si sigue COMPRADO) y limpia purchased_by y purchased_at.
   La escritura de position pasa toda por lib/reorder.ts, por la misma razón que la de status: un PATCH genérico que aceptara position dejaría dos filas en el mismo sitio, y eso no salta como error — aparece semanas después como un baile de tarjetas en cada recarga. El endpoint es propio (PATCH de /api/admin/lists/order y de /api/admin/lists/[listId]/products/order) y recibe TODOS los ids, no solo el que se movió: es lo que permite detectar que el navegador estaba viendo otra cosa (el otro padre acaba de añadir un regalo) y responder 409 en vez de reescribir un orden con huecos. Las posiciones se renumeran desde 0 dentro de una transacción.
   Una lista oculta es invisible, no "no listada": para quien no sea uno de los padres da el MISMO 404 que un slug inventado, en el índice, en su enlace, en el enlace de cada regalo y al meter la clave. Un 403 del tipo "está oculta" confirmaría que existe. La regla vive en esVisible/findVisibleListBySlug (lib/lists/queries.ts), y el nombre lleva "Visible" para que un findListBySlug a secas se note que se salta algo. El índice público no las enseña NI a los padres a propósito: esa pantalla es la forma de comprobar de un vistazo que ocultar surtió efecto.
...
8. Trampas conocidas / decisiones que no hay que revisar sin motivo
   ...
   No crear entorno de pre hasta que un despliegue arriesgado o una producción rota lo justifiquen (disparador real, no por defecto). Esto es lo que hace vercel.json, que no puede explicarse a sí mismo porque el JSON no admite comentarios: deploymentEnabled desactiva los despliegues de todas las ramas menos main, porque hay UNA sola base de datos y un preview de una rama escribiría en la de producción. Y regions: fra1 pone las funciones en Fráncfort, donde está la base (Neon, eu-central-1), en vez de en Washington, que es el destino por defecto.
   No hashear la clave de acceso de lista — es intencional, no un olvido.
   No meter @dnd-kit (ni otra librería de arrastrar y soltar) para reordenar. Se miró en septiembre de 2026: cero CVEs y licencia MIT, pero la rama estable (v6) llevaba sin publicarse desde diciembre de 2024 y nunca declaró soportar React 19, y el sucesor seguía en 0.x. Lo que hacía falta era mover un puñado de tarjetas en una pantalla que usan dos personas, así que está escrito a mano sobre Pointer Events en lib/hooks/use-ordenable.ts. Pointer Events y no el arrastre nativo de HTML5 porque aquel no funciona con el dedo en iOS ni en Android, y el panel se usa desde el móvil.
